...
CHANNELS_TO_SUB = [
    {
        "id":   "-1003414162996",
        "link": "https://t.me/+_JlUsLUAJlkyYWI0",
...
# ── Умная разбивка текста по границам строк ─────────────────
 
def split_smart(text: str, max_len: int) -> list[str]:
    """
    Режет текст на части не длиннее max_len символов.
    Старается резать по переносу строки, а не посередине слова.
    """
    chunks = []
...
# ── Генерация со стримингом и эффектом появления текста ─────
 
def generate_and_stream(chat_id: int, msg_id: int, messages_payload: list) -> str | None:
    """
    Генерирует ответ с реальным стримингом.
 
    Как работает эффект:
    - Текст накапливается по частям из стрима модели.
    - Каждые STREAM_UPDATE_INTERVAL секунд сообщение редактируется —
      пользователь видит как текст появляется кусками в реальном времени.
    - В конце каждого промежуточного обновления добавляется курсор █.
    - После завершения генерации курсор убирается, отправляется финальный текст.
 
    Возвращает полный raw-текст ответа (до конвертации) или None.
    """
    STREAM_UPDATE_INTERVAL = 1.2   # секунд между редактированиями
    MAX_STREAM_LEN         = 3800  # лимит для промежуточных сообщений (с запасом)
    ...
    for attempt in range(len(API_KEYS)):
        ...
        try:
            ...
            for part in client.chat(
                ...
                now = time.time()
                if now - last_update_at >= STREAM_UPDATE_INTERVAL:
                    try:
                        # Берём последние MAX_STREAM_LEN символов если уже много текста
                        display = full_response
                        if len(display) > MAX_STREAM_LEN:
                            display = "…" + display[-MAX_STREAM_LEN:]
 
                        # Промежуточный текст — plain text, без MarkdownV2.
                        # Конвертировать незавершённый Markdown опасно (незакрытые теги).
                        bot.edit_message_text(
                            display + " █",
                            chat_id, msg_id
                        )
                    except Exception:
                        pass  # "message is not modified" и прочие — пропускаем
                    last_update_at = now
...
def send_ai_response(chat_id: int, status_msg_id: int, text: str, cost: int):
    ...
    Пробует MarkdownV2 → при ошибке падает на plain text.
    Длинные ответы разбивает на части по границам строк.
    """
    MAX_LEN     = 4000
    cost_suffix = f"\n\n💰 \\-{cost} Purrs"
    ...
    # Длинный ответ — режем и отправляем частями
...
def ai_message_handler(message):
    ...
    # 7. Начальное сообщение — сразу покажем █ пока не пришёл первый чанк
    status_text = "🔍 Analyzing █" if is_photo else "✨ Thinking █"
    status_msg  = bot.reply_to(message, status_text)
 
    # 8. Генерация со стримингом (редактирует status_msg по ходу)
    full_response = generate_and_stream(
        message.chat.id, status_msg.message_id, messages_to_send
    )
    ...
    # 10. Заменяем стриминговое сообщение на финальное с форматированием
...
INACTIVITY_MESSAGES = [
    ...
    "✨ Just checking in!\nHaven't chatted in a bit — what's going on?",
    "😺 Psst... remember me?\nCome back, I'm bored without you.",
    "🌙 Been quiet lately.\nFeel free to drop a message anytime — I'm always around.",
]
 
def get_last_activity(uid: str) -> datetime.datetime | None:
    """Возвращает дату последнего сообщения пользователя из таблицы chats."""
    row = db_query(
        "SELECT timestamp FROM chats WHERE uid = ? ORDER BY id DESC LIMIT 1",
        (uid,), fetchone=True
    )
    if not row:
        return None
...
def check_inactivity():
    ...
    Окно 48–72 часа: отправляем ровно одно напоминание за этот период,
    не спамим каждый час.
    """
    now       = datetime.datetime.now()
    all_users = db_query("SELECT uid FROM users", fetchall=True)
    if not all_users:
        return
 
    for (uid,) in all_users:
        try:
            last_active = get_last_activity(uid)
            if not last_active:
                continue
            ...
            if 48 <= hours_ago < 72:
                ...
                send_log("INACTIVITY", f"📬 Sent reminder to `{uid}`")
 
        except Exception as e:
            print(f"⚠️ Inactivity check failed for {uid}: {e}")
 
def inactivity_scheduler():
    """Фоновый поток — проверяет неактивность раз в час."""
    while True:
        time.sleep(3600)
        try:
            check_inactivity()
...
# Запускаем планировщик сразу при старте
threading.Thread(
    target=inactivity_scheduler,
    daemon=True,
    name="InactivityScheduler"
).start()
